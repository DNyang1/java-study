...
// 5) 프로젝트 생성 후 `src` 폴더 아래에 `board` 패키지 아래에 `DBConnection.java` 생성
...
// DB의 테이블 구조와 대응되는 순수한 데이터 전달용 객체
// DB 테이블의 한 행(row)을 객체로 표현할 때 자주 사용
// DB(레코드) ---> 자바(객체)
...
// 1) DTO(Board.java) - 게시글 정보를 담을 객체, lombok 설치 및 적용(설치 후 IDE 재시작)
