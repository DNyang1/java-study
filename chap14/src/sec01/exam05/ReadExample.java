...
public class ReadExample {
	...
	public static void main(String[] args) throws Exception {
		...
		// 한번에 지정한 배열 크기만큼 바이트 데이터를 읽어 들이므로 훨씬 빠르게 읽기가능
		byte[] buffer = new byte[100];
		
		while (true) {
			// 입력 스트림으로 부터 배열의 길이만큼 읽어옴
			int readByteNum = is.read(buffer); // 배열의 길이보다 적을 경우는 그냥 읽을수 있을만큼 읽어옴
			if (readByteNum == -1) break;

			// 읽은 바이트 수 만큼 반복하는게 맞음
