...
public class LotteryController {
	...
	public Set<Lottery> winObject(){
		...
		List<Lottery> lotList = new ArrayList<Lottery>(lottery);  
		...
		while (win.size()==4) {
			int randNum = (int)(Math.random() * lottery.size());
			if (!win.contains(lotList.get(randNum))) {
//				if (condition) {
//					win.add(lotList.get(randNum));
//				}
			}
